...
def _make_dataset(n: int, *, drift: bool = False) -> pd.DataFrame:
    ...
    monthly_charges = monthly_charges_base + (10 if drift else 0)  # drift: precios subieron

    total_charges = monthly_charges * tenure * RNG.uniform(0.9, 1.1, n)
    num_products = RNG.integers(1, 6, n)
    support_tickets = RNG.poisson(1.5 if not drift else 2.5, n)  # drift: más tickets
    logins = RNG.poisson(15 if not drift else 10, n)  # drift: menos logins
    session = RNG.gamma(2, 5, n).clip(0.5, 60)

    contract = RNG.choice(
        CONTRACT_TYPES,
        n,
        p=[0.5, 0.3, 0.2] if not drift else [0.65, 0.2, 0.15],  # drift: más month-to-month
    )
